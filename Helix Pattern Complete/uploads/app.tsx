...
const App = () => {
  ...
  // Pass rotation as an argument
  const project3D = (point, canvasWidth, canvasHeight, currentRotation) => {
    const scale = 150;
    const centerX = canvasWidth / 2;
    const centerY = canvasHeight / 2;
    
    // Apply rotation from argument
  ...
  // Ref to hold current state for the animation loop to prevent stale closures
  const stateRef = useRef();
  
  // Use layout effect to update the ref synchronously after render but before paint
  // This avoids mutating the ref during the render phase, which can be unsafe
  useLayoutEffect(() => {
    stateRef.current = {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      // Read all state from the ref to prevent stale closures
      ...
      // Use rotation from the state ref
      const projectedPoints = currentState.helixPoints.map((point, index) => ({
        ...point,
        originalIndex: index, // Store the original index
        projected: project3D(point, width, height, currentState.rotation)
      }))
      .filter(p => p.projected.z > 0.1) // Filter points behind or too close to camera
      .sort((a, b) => a.projected.z - b.projected.z);
      
      // Draw connections between adjacent points
      if (currentState.showConnections) { // Use state from ref
      ...
      // Draw points
      projectedPoints.forEach((point) => { // 'index' is not needed here
        ...
        if (point.isSpecial) {
          size += Math.sin(currentState.pulsePhase) * 2; // Use state from ref
          
          // Draw rings around special point
          for (let ring = 1; ring <= 3; ring++) {
            const ringRadius = size + (ring * 8) + Math.sin(currentState.pulsePhase + ring) * 3; // Use state from ref
            ctx.strokeStyle = `hsla(${point.hue}, 80%, 60%, ${0.3 / ring})`;
            ctx.lineWidth = 2 / ring;
            ctx.beginPath();
            ctx.arc(p.x, p.y, Math.max(0.1, ringRadius), 0, Math.PI * 2); // Clamp radius
            ctx.stroke();
          }
        }
        
        // Hover effect
        if (currentState.hoveredPoint === point.originalIndex) { // Use originalIndex
          size *= 1.5;
          ctx.shadowBlur = 20;
          ctx.shadowColor = `hsl(${point.hue}, 100%, 70%)`;
        } else if (currentState.selectedPoint === point.originalIndex) { // Use originalIndex
          size *= 1.3;
          ctx.shadowBlur = 15;
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, Math.max(0.1, size), 0, Math.PI * 2); // Clamp radius
        ctx.fill();
        
        ctx.shadowBlur = 0;
        
        // Draw coordinate label for special point
        if (point.isSpecial && (currentState.selectedPoint === point.originalIndex || currentState.hoveredPoint === point.originalIndex)) { // Use originalIndex
      ...
      // Draw axes in corner
      drawAxes(ctx, width, height, currentState.rotation); // Pass rotation from ref
  ...
  }, [helixPoints]); // This dependency array is stable and won't cause a loop
  
  // Draw coordinate axes
  // Pass rotation as an argument
  const drawAxes = (ctx, width, height, currentRotation) => {
    const axisLength = 40;
    const origin = { x: 60, y: height - 60 };
    
    // Project axis endpoints
    const xAxis = project3D({ x: 1, y: 0, z: 0 }, width, height, currentRotation);
    const yAxis = project3D({ x: 0, y: 1, z: 0 }, width, height, currentRotation);
    const zAxis = project3D({ x: 0, y: 0, z: 1 }, width, height, currentRotation);
    const originProj = project3D({ x: 0, y: 0, z: 0 }, width, height, currentRotation);
  ...
  const handleMouseDown = (e) => {
    ...
    // Note: We reverse the sorted array to check front points first
    // Use current rotation from state for click detection
    const projectedPoints = helixPoints.map((point, index) => ({
        ...point,
        projected: project3D(point, width, height, rotation),
        originalIndex: index
      })).sort((a, b) => b.projected.z - a.projected.z); // Sort far to near
    ...
    for (const point of projectedPoints) {
        ...
        if (dist < size + 5) { // 5px buffer
            clickedPointIndex = point.originalIndex;
            break; // Found the topmost clicked point
        }
    }
    
    if (clickedPointIndex !== -1) {
      setSelectedPoint(clickedPointIndex);
      if (helixPoints[clickedPointIndex].isSpecial) {
        setResonanceLevel(1.0);
      }
    } else {
      setSelectedPoint(null); // Deselect if clicking background
      setIsDragging(true);
  ...
  const handleMouseMove = (e) => {
    ...
    if (isDragging) {
      ...
      // Check hover
      // Use current rotation from state for hover detection
      const projectedPoints = helixPoints.map((point, index) => ({
        ...point,
        projected: project3D(point, width, height, rotation),
        originalIndex: index
  ...
  return (
    <div className="relative w-full h-screen bg-slate-950 overflow-hidden font-mono">
      ...
      <canvas
        ref={canvasRef}
        // Remove fixed width/height, let CSS and resize handler control it
      ...
      {/* Memory Display */}
      {selectedMemory && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 w-[90vw] max-w-[600px]">
